...
class SupConLoss_clear(nn.Module):
    ...
    def forward(self, features, labels):
        ...
        batch_size = features.shape[0]  # 2048
        labels = labels.contiguous().view(-1, 1)
        mask = torch.eq(labels, labels.T).float().to(device)

        anchor_dot_contrast = torch.div(
            torch.matmul(features, features.T),
            self.temperature)  # 2048*2048

        # normalize the logits for numerical stability
        logits_max, _ = torch.max(anchor_dot_contrast, dim=1, keepdim=True)
        logits = anchor_dot_contrast - logits_max.detach()

        logits_mask = torch.scatter(  # negative identity matrix
            torch.ones_like(mask),
            1,
            torch.arange(batch_size).view(-1, 1).to(device),
            0
        ...
        mask = mask * logits_mask
        single_samples = (mask.sum(1) == 0).float()  # if there only label in one batch then return true else return false

        # compute log_prob
        exp_logits = torch.exp(logits)*logits_mask  # make diagonal = 0
        log_prob = logits - torch.log(exp_logits.sum(1, keepdim=True))
        ...
        loss = - mean_log_prob_pos*(1-single_samples)  # filter the only label in one batch
